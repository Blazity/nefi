...
There are some command parameters that can be passed to the `nefi` command:

- `--verbose` - enables verbose logging. Useful for debugging and troubleshooting.
  Please refer to the [reporting an issue](/docs/usage/reporting-an-issue) guide for more information.
  
- `--usage` - prints the usage of the Anthropic API in tokens